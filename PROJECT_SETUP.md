...
- `AGENTS.md`、`CONTEXT.md` 與 `docs/adr/` 是 project-local 實體內容。
...
初始化產生的 `.agents/AGENTS.md` 是穩定的小型入口：
...
完整共享規則不會複製進專案，因此 rules 不會形成過期 snapshot。
...
- 不覆寫既有 `AGENTS.md`、`CONTEXT.md` 或 ADR。
